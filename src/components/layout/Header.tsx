...
export default function Header({
  ...
  return (
    <header
      className="h-16 border-b border-[#1E2330] bg-[#111318]/90 backdrop-blur-md px-4 sm:px-6 flex items-center justify-between sticky top-0 z-40"
      id="fm-header"
    >
        ...
        {focusModeActive ? (
          ...
          <div className="flex items-center gap-2">
            <h1 className="text-sm sm:text-lg font-bold tracking-tight text-white flex items-center gap-2">
              ...
              <span>FlowMind</span>
              <span className="hidden xs:inline text-[10px] sm:text-xs font-normal text-slate-400 font-mono tracking-normal bg-[#181C27] border border-[#1E2330] px-1.5 py-0.5 rounded">
                v2.5
              </span>
            </h1>
          </div>
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div className="hidden md:flex items-center gap-2 py-1 px-2.5 rounded-full bg-[#181C27] border border-[#1E2330]">
          {orchestrating ? (
            <>
              ...
              <span className="font-mono text-[10px] text-[#7C6AF7] font-semibold uppercase tracking-wider animate-pulse">
                Orchestrating...
              </span>
            </>
          ) : apiStatus?.active ? (
        ...
        <div className="flex items-center gap-2" id="header-auth-controls">
          {user ? (
            <div className="flex items-center gap-2">
              <div
                className="flex items-center gap-2 py-1 pl-1 pr-2 rounded-full bg-[#111318] border border-[#1E2330]"
                title={user.email}
              >
                {user.photoURL ? (
                  <img

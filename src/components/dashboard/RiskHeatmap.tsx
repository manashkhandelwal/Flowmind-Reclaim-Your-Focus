...
interface RiskHeatmapProps {
  ...
  onOrchestrate: (trigger: "initial_sync" | "gmail_scan" | "daily_briefing", unstructuredFeed?: string) => void;
...
export default function RiskHeatmap({
  ...
  const getRiskColor = (level: string) => {
    switch (level) {
      case "high_risk":
        return "text-[#F04438] bg-[#F04438]/10 border-[#F04438]/20";
      case "at_risk":
        return "text-[#F5A623] bg-[#F5A623]/10 border-[#F5A623]/20";
      default:
        return "text-[#34C77B] bg-[#34C77B]/10 border-[#34C77B]/20";
    }
  };

  const getUrgencyMetricLabel = (deadlineStr: string) => {
    const hoursLeft =
      (new Date(deadlineStr).getTime() - Date.now()) / (1000 * 60 * 60);
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#1E2330]/60 pb-3 mb-5">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Calculated based on estimated effort vs. real-time deadline
              buffers.
            </p>
          </div>
          ...
          >
            <RefreshCw
              size={11}
              className={orchestrating ? "animate-spin" : ""}
            />
            Re-Score Matrix
          </button>
        </div>
        ...
        >
          ...
          <div className="border border-red-500/10 rounded-xl bg-red-500/[0.01] p-4 flex flex-col gap-3">
            <div className="flex items-center justify-between pb-1 border-b border-[#1E2330]/50">
              ...
              <span className="text-[10px] font-mono text-slate-500 bg-[#111318] px-1.5 rounded">
                {activeTasks.filter((t) => t.riskLevel === "high_risk").length}{" "}
                tasks
              </span>
            </div>
            ...
            <div className="space-y-2 flex-grow overflow-y-auto max-h-[280px]">
              ...
              0 ? (
                ...
                activeTasks
                  .filter((t) => t.riskLevel === "high_risk")
                  .map((task) => (
                    <div
                      key={task.id}
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <span className="text-xs font-bold text-slate-200 line-clamp-1 group-hover:text-[#4F8EF7]">
                          {task.title}
                        </span>
          ...
          <div className="border border-amber-500/10 rounded-xl bg-amber-500/[0.01] p-4 flex flex-col gap-3">
            <div className="flex items-center justify-between pb-1 border-b border-[#1E2330]/50">
              ...
              <span className="text-[10px] font-mono text-slate-500 bg-[#111318] px-1.5 rounded">
                {activeTasks.filter((t) => t.riskLevel === "at_risk").length}{" "}
                tasks
              </span>
            </div>
          ...
          <div className="border border-green-500/10 rounded-xl bg-green-500/[0.01] p-4 flex flex-col gap-3">
            ...
            <div className="space-y-2 flex-grow overflow-y-auto max-h-[280px]">
              {activeTasks.filter((t) => t.riskLevel === "safe").length ===
              0 ? (
                <div className="h-24 flex items-center justify-center border border-dashed border-[#1E2330] rounded-lg text-slate-500 text-xs font-mono">
                  No shielded tasks.
                </div>
